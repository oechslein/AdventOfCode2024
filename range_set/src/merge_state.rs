...
impl<A: Array, B: Array> MergeState for InPlaceMergeState<'_, A, B> {
    type A = A::Item;
    type B = B::Item;
...
impl<A: Array> MergeStateMut for InPlaceMergeState<'_, A, A> {
    #[inline]
    fn advance_a(&mut self, n: usize, take: bool) -> bool {
        self.ac ^= is_odd(n);
...
impl<A: Array, B> MergeState for InPlaceMergeStateRef<'_, A, B> {
    type A = A::Item;
    type B = B;
    fn a_slice(&self) -> &[A::Item] {
        self.a.source_slice()
...
impl<A: Array> MergeStateMut for InPlaceMergeStateRef<'_, A, A::Item>
where
    A::Item: Clone,
{
    #[inline]
...
impl<A: Debug, B: Debug> Debug for BoolOpMergeState<'_, A, B> {
...
impl<A, B> MergeState for BoolOpMergeState<'_, A, B> {
    type A = A;
    type B = B;
    fn a_slice(&self) -> &[A] {
        self.a.as_slice()
...
impl<A, B> MergeStateMut for BoolOpMergeState<'_, A, B> {
    fn advance_a(&mut self, n: usize, take: bool) -> bool {
        self.ac ^= is_odd(n);
        if take {
            self.r = true;
...
pub(crate) struct SmallVecMergeState<'a, A, B, T> {
    a: SliceIterator<'a, A>,
    b: SliceIterator<'a, B>,
    ac: bool,
    bc: bool,
...
impl<A: Debug, B: Debug, T> Debug for SmallVecMergeState<'_, A, B, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "a: {:?}, b: {:?}", self.a_slice(), self.b_slice(),)
    }
}

impl<'a, A, B, T> SmallVecMergeState<'a, A, B, T> {
    pub fn new(a: &'a [A], b: &'a [B], r: SmallVec<[T; 2]>) -> Self {
        Self {
            a: SliceIterator(a),
            b: SliceIterator(b),
            ac: false,
    ...
    pub fn result(self) -> SmallVec<[T; 2]> {
        self.r
    }

    pub fn merge<O: MergeOperation<Self>>(a: &'a [A], b: &'a [B], o: O) -> SmallVec<[T; 2]> {
        let t: SmallVec<[T; 2]> = SmallVec::new();
...
impl<A, B, T> MergeState for SmallVecMergeState<'_, A, B, T> {
    type A = A;
    type B = B;
    fn a_slice(&self) -> &[A] {
        self.a.as_slice()
...
impl<T: Clone> MergeStateMut for SmallVecMergeState<'_, T, T, T> {
    fn advance_a(&mut self, n: usize, take: bool) -> bool {
        self.ac ^= is_odd(n);
        if take {
            self.r.reserve(n);
