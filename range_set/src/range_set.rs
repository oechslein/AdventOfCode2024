...
use std::num::*;
use std::ops::{Deref, RangeBounds, RangeFull, RangeInclusive};
...
pub struct RangeSet<T>(SmallVec<[T; 2]>);
...
impl<T: Clone> RangeSetElement<&T> {
    /// Maps a `RangeSetRange<&T>` to a `RangeSetRange<T>` by cloning start and end.
    pub fn cloned(&self) -> RangeSetElement<T> {
        match self {
            RangeSetElement::Range(r) => RangeSetElement::Range(r.start.clone()..r.end.clone()),
            RangeSetElement::RangeFrom(r) => RangeSetElement::RangeFrom(r.start.clone()..),
        }
    }
}

impl<T> From<Range<T>> for RangeSetElement<T> {
    fn from(r: Range<T>) -> Self {
        RangeSetElement::Range(r)
    }
}

impl<T> From<RangeFrom<T>> for RangeSetElement<T> {
    fn from(r: RangeFrom<T>) -> Self {
        RangeSetElement::RangeFrom(r)
    }
}

impl<T: RangeSetEntry> From<RangeSetElement<T>> for RangeSet<T> {
    fn from(value: RangeSetElement<T>) -> Self {
        match value {
            RangeSetElement::Range(r) => RangeSet::from(r),
            RangeSetElement::RangeFrom(r) => RangeSet::from(r),
        }
    }
}

impl<T: Debug> Debug for RangeSetElement<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RangeSetElement::Range(r) => r.fmt(f),
            RangeSetElement::RangeFrom(r) => r.fmt(f),
        }
    }
}

impl<T> RangeBounds<T> for RangeSetElement<T> {
    fn start_bound(&self) -> Bound<&T> {
        match self {
            RangeSetElement::Range(r) => r.start_bound(),
            RangeSetElement::RangeFrom(r) => r.start_bound(),
        }
    }

    fn end_bound(&self) -> Bound<&T> {
        match self {
            RangeSetElement::Range(r) => r.end_bound(),
            RangeSetElement::RangeFrom(r) => r.end_bound(),
        }
    }
}
...
impl<T: RangeSetEntry, R: AsRef<RangeSetRef<T>>> PartialEq<R> for RangeSet<T> {
    fn eq(&self, that: &R) -> bool {
        self.boundaries() == that.as_ref().boundaries()
    }
}

impl<T: Eq + RangeSetEntry> Eq for RangeSet<T> {}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if !bounds.is_empty() {
            Some(if bounds.len() == 1 {
                self.0 = &bounds[1..];
                RangeSetElement::from(&bounds[0]..)
            } else {
                self.0 = &bounds[2..];
                RangeSetElement::from(&bounds[0]..&bounds[1])
            })
        } else {
            None
        }
...
impl<T> RangeSetRef<T> {
    ...
    /// intersection
    pub fn intersection(&self, that: &RangeSetRef<T>) -> RangeSet<T>
    where
        T: Ord + Clone,
    {
    ...
    /// union
    pub fn union(&self, that: &RangeSetRef<T>) -> RangeSet<T>
    where
        T: Ord + Clone,
    {
        RangeSet::new_unchecked_impl(VecMergeState::merge(
    ...
    /// difference
    pub fn difference(&self, that: &RangeSetRef<T>) -> RangeSet<T>
    where
        T: Ord + Clone,
    {
        RangeSet::new_unchecked_impl(VecMergeState::merge(
    ...
    pub fn symmetric_difference(&self, that: &RangeSetRef<T>) -> RangeSet<T>
    where
        T: Ord + Clone,
    {
        RangeSet::new_unchecked_impl(VecMergeState::merge(
...
impl<T> RangeSet<T> {
    ...
    /// If you want to avoid this check, use `new_unchecked`
    /// (behind a feature flag because it is unsafe)
    pub fn new(boundaries: SmallVec<[T; 2]>) -> Option<Self>
    where
        T: Ord,
    {
    ...
    /// strictly sorted.
    #[cfg(feature = "new_unchecked")]
    pub fn new_unchecked(boundaries: SmallVec<[T; 2]>) -> Self {
        Self::new_unchecked_impl(boundaries)
    }

    /// note that this is private since it does not check the invariants!
    fn new_unchecked_impl(boundaries: SmallVec<[T; 2]>) -> Self {
        RangeSet(boundaries)
    }
...
impl<T: RangeSetEntry> RangeSet<T> {
...
impl<T: PrimInt + Sized + Sum<T>> RangeSet<T> {
    /// length of a range set
    pub fn len(&self) -> T {
        self.iter().map(|range| range.len()).sum()
    }
}

impl<T: PrimInt> RangeSetElement<&T> {
    /// length of a range
    fn len(&self) -> T {
        match self {
            RangeSetElement::Range(curr_range) => *curr_range.end - *curr_range.start,
            RangeSetElement::RangeFrom(_) => T::max_value(),
        }
    }
}

impl<T: RangeSetEntry + Clone> RangeSet<T> {
...
impl<T: RangeSetEntry> From<bool> for RangeSet<T> {
...
impl<T: RangeSetEntry + PrimInt> RangeSet<T> {
    fn from_range_inclusive(a: RangeInclusive<T>) -> Self {
        Self::from_range(*a.start()..(*a.end() + T::one()))
    }
}

impl<T: RangeSetEntry + PrimInt> From<RangeInclusive<T>> for RangeSet<T> {
    fn from(value: RangeInclusive<T>) -> Self {
        Self::from_range_inclusive(value)
    }
}

impl<T: RangeSetEntry> From<Range<T>> for RangeSet<T> {
    fn from(value: Range<T>) -> Self {
        Self::from_range(value)
    }
}

impl<T: RangeSetEntry> From<RangeFrom<T>> for RangeSet<T> {
    fn from(value: RangeFrom<T>) -> Self {
        Self::from_range_from(value.start)
    }
}

impl<T: RangeSetEntry> From<RangeTo<T>> for RangeSet<T> {
    fn from(value: RangeTo<T>) -> Self {
        Self::from_range_until(value.end)
    }
}

impl<T: RangeSetEntry> From<RangeFull> for RangeSet<T> {
...
impl<T: RangeSetEntry + Clone> BitAnd<RangeSet<T>> for RangeSet<T> {
    type Output = RangeSet<T>;
    fn bitand(self, that: RangeSet<T>) -> Self::Output {
...
///
/// &forall; t &isin; T, r(t) = a(t) | b(t)
impl<T: RangeSetEntry + Clone> BitOr<RangeSet<T>> for RangeSet<T> {
    type Output = RangeSet<T>;
    fn bitor(self, that: RangeSet<T>) -> Self::Output {
        self.union(&that)
    }
}
...
///
/// &forall; t &isin; T, r(t) = a(t) ^ b(t)
impl<T: RangeSetEntry + Clone> BitXor<RangeSet<T>> for RangeSet<T> {
    type Output = RangeSet<T>;
    fn bitxor(self, that: RangeSet<T>) -> Self::Output {
...
impl<T: RangeSetEntry> BitXorAssign for RangeSet<T> {
...
///
/// &forall; t &isin; T, r(t) = a(t) & !b(t)
impl<T: RangeSetEntry + Clone> Sub<RangeSet<T>> for RangeSet<T> {
    type Output = RangeSet<T>;
    fn sub(self, that: RangeSet<T>) -> Self::Output {
...
impl<T: RangeSetEntry + Clone> Not for RangeSet<T> {
    type Output = RangeSet<T>;
...
impl<T> MergeStateMut for RangeSetBoolOpMergeState<'_, T> {
...
impl<T> MergeState for RangeSetBoolOpMergeState<'_, T> {
    type A = T;
    type B = T;
    fn a_slice(&self) -> &[T] {
        self.inner.a_slice()
...
struct VecMergeState<'a, T> {
    inner: SmallVecMergeState<'a, T, T, T>,
}

impl<'a, T: Clone> VecMergeState<'a, T> {
    fn merge<O: MergeOperation<Self>>(a: &'a [T], b: &'a [T], o: O) -> SmallVec<[T; 2]> {
        let mut state = Self {
...
impl<T: Clone> MergeStateMut for VecMergeState<'_, T> {
    fn advance_a(&mut self, n: usize, copy: bool) -> bool {
        self.inner.advance_a(n, copy)
    }
    fn advance_b(&mut self, n: usize, copy: bool) -> bool {
...
impl<T> MergeState for VecMergeState<'_, T> {
    type A = T;
    type B = T;
    fn a_slice(&self) -> &[T] {
        self.inner.a_slice()
...
#[cfg(feature = "serde")]
impl<T: Serialize> Serialize for RangeSet<T> {
...
#[cfg(feature = "serde")]
impl<'de, T: Deserialize<'de> + Ord> Deserialize<'de> for RangeSet<T> {
...
#[cfg(feature = "serde")]
impl<'de, T> Visitor<'de> for RangeSetVisitor<T, A>
where
    T: Deserialize<'de> + Ord,
{
        ...
        B: SeqAccess<'de>,
    {
        ...
        let mut boundaries: SmallVec<[T; 2]> = SmallVec::with_capacity(len);
...
#[cfg(feature = "rkyv")]
impl<T, A> rkyv::Archive for RangeSet<T>
where
    T: rkyv::Archive,
{
...
#[cfg(feature = "rkyv")]
impl<T, S, A> rkyv::Serialize<S> for RangeSet<T>
where
...
#[cfg(feature = "rkyv")]
impl<T, A, D> rkyv::Deserialize<RangeSet<T>, D> for ArchivedRangeSet<T::Archived>
where
    T: rkyv::Archive,
...
{
    fn deserialize(&self, deserializer: &mut D) -> Result<RangeSet<T>, D::Error> {
...
mod tests {
    ...
    impl<T: Arbitrary + Ord + Clone + 'static> Arbitrary for RangeSet<T> {
    ...
    impl<E: PrimInt + RangeSetEntry + Clone> TestSamples<E, bool> for RangeSet<E> {
